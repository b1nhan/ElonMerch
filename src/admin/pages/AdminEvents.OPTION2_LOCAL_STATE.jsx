/**
 * AdminEvents.jsx - OPTION 2: OPTIMISTIC LOCAL STATE UPDATES
 * 
 * This approach updates the UI immediately with the new data without
 * waiting for a re-fetch. Uses setEvents with functional updates.
 * 
 * Pros: Instant UI update, faster perceived performance, fewer network requests
 * Cons: Requires careful handling if server modifies data differently than client
 * 
 * Best for: CRUD operations that don't have complex server-side logic,
 * when user experience speed is critical
 */
...
const AdminEvents = () => {
   ...
   * Only called on mount and when explicitly needed
   */
  ...
  /**
   * OPTION 2: HANDLE CREATE WITH OPTIMISTIC LOCAL STATE UPDATE
   * 
   * Flow:
   * 1. Send API request (POST)
   * 2. Immediately add new event to state (optimistic update)
   * 3. Wait for response to get real data from server
   * 4. If response contains created event data, replace optimistic with real data
   * 5. If error, remove from state and show error message
   */
  const handleSaveEvent = async (formData) => {
    try {
      ...
      if (editingEvent?.id) {
        ...
        // Verify response contains updated event
        ...
        setEvents(prevEvents => 
          prevEvents.map(event => 
            event.id === editingEvent.id 
              ? { ...event, ...response.data }  // Replace with server data
              : event
          )
        );
        ...
        // ====================================================================
        // HANDLE CREATE WITH LOCAL STATE
        // ====================================================================
        // Send CREATE request
        const response = await apiPost('/events', formData);
        
        // Verify response contains created event with ID
        if (!response.data || !response.data.id) {
          throw new Error('Backend did not return created event data');
        }

        // Add created event to state
        // Uses functional update to ensure we're adding to latest state
      ...
      // IMPORTANT: On error, re-fetch to ensure state is in sync with server
      // This prevents UI and database from getting out of sync
      await fetchEvents();
  ...
  /**
   * OPTION 2: HANDLE DELETE WITH OPTIMISTIC LOCAL STATE UPDATE
   * 
   * Flow:
   * 1. Remove event from state immediately (optimistic delete)
   * 2. Send DELETE request
   * 3. If error, add event back to state
   */
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      // Show success notification (before confirmation)
      setNotification({ 
        type: 'success', 
      ...
      try {
        ...
        // DELETE succeeded - event is already removed from state ✓
      } catch (deleteErr) {
        console.error('Delete request failed:', deleteErr);
        
        // ====================================================================
        // ERROR HANDLING: Add event back if DELETE failed
        // ====================================================================
        setEvents(prevEvents => {
          // Add the event back in its original position (approximate)
          const updatedEvents = [...prevEvents];
          updatedEvents.push(eventToDelete);
