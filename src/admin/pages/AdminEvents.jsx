...
const AdminEvents = () => {
  ...
  try {
    setModalLoading(true);
    const res = await apiDelete(`/events/${deleteConfirm.id}`); //

    // CHỐT HẠ: Phải kiểm tra status từ Backend trả về!
    if (res.status === 'success') {
      setNotification({ type: 'success', message: 'Event deleted successfully!' });
      setDeleteConfirm(null);
      await fetchEvents(); // Load lại danh sách mới
    } else {
      // Nếu Backend báo lỗi (vd: ID không tồn tại), hiện lỗi thật lên
      setNotification({ type: 'error', message: res.message || 'Failed to delete' });
    }
  } catch (err) {
    setNotification({ type: 'error', message: 'Network error or Server down' });
  } finally {
    setModalLoading(false);
  }
};
